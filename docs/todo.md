...
- 数据仍刷新不充分：需建立后端的【学习工作区创建，大纲、复习卡、工件呈现、笔记什么的有变】就推送前端刷新的机制。或者做成某种反应式数据源，后端管理，前端仅呈现和发命令

- 应有专门文件收集Dvl Labels；然后是Util/包的建立

- 落盘纲：parentId 需要用空字符串而不是 null。bug？
- 落盘纲：要不要有agent对纲、课的额外批注？（掌握情况，明文目标...）；课可绑定工件了？而且为什么状态机不是在纲（total:lesson-x:in-progress）而是每可课一个？？？
- 会话的active纲目保存在...？不应该工作区，而应保存在会话（但纲目进展确保存在工作区）！会话latest的纲目激活事件，决定该会话的激活纲目是哪个：基本休了
- 纲的数据结构/拓扑。树状就蛮好，不要目前这个铺平。以及总恩情状态机
- 要激活的纲目，才给这个会话注入其复习

- 没看到调用update_review？

- 老注入learning上下文？（一工具调用就注入）

- 纲目等东西的update失败，要在工具返回的作为一种密封类型。另外，每 turn 检测的阻塞最好返给 AI，而不是直接抛
