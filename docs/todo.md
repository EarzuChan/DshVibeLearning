...
- 工件内：工件为什么会出现“提交按钮”然后又“完成按钮”，应改得只有一个。工件没有alert提交反馈（设个timer，好让独立窗口展示，而内置在timer前掐了。或者是submit的提交返回值来指定alert）【后话】
- fdbk可让agent不再往report中复述题干【后话】
...
- 所有的工件，以后最好提供edit工件的工件，统一新建写入和编辑已有。免得模型创建tmp（很容易烦他）->计算哈希->复制。这是个大流程，需要从长计议【后话】

- 优化Prompt和引导性。现在凑合能用但有时会卡卡。另外，状态机可能有卡住风险，是否需要搞一个安全逃生模式（让模型强制跳去哪里，并让工具弹窗由用户确认）【后话】

- 前端刷新还是有点不及时，尤其是：进学习->建纲->激活纲，都没刷新ui（学习tab以及当前激活。我手动触刷新才正常）怕第二阶段反覆【后话】
