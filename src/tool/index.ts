...
export function installLearningTools(rootCtx: Context, toolCtx: Context, agent: Agent): () => void {
    ...
    disposers.push(toolCtx.tools.register(defineTool({
        ...
        description: `在会话内呈现一个已存在的学习工件并等待本次 Run 终局。工件须位于 <workspace>/${LEARNING_ARTIFACT_PATH}。本工具只创建或复用 Run、取得 In-band 占用并等待；绝不改变大纲 Workflow 或复习计划。等待超时或调用中断不会终结 Run。`,
    ...
    disposers.push(toolCtx.tools.register(defineTool({
        ...
        async execute(args: {outline_id: string | null; title: string; tree: JsonValue}, exec) {
            ...
            try {
                ...
                const answer = await confirmWrite(rootCtx, agent, args.outline_id === null ? `新建大纲「${args.title}」？` : `更新大纲「${args.title}」？`, JSON.stringify(outline.tree, null, 2), exec.signal)
    ...
    disposers.push(toolCtx.tools.register(defineTool({
        ...
        async execute(args: {outline_id: string; lesson_id: string; artifact_hash: string | null}, exec) {
            ...
            try {
                ...
                const answer = await confirmWrite(rootCtx, agent, args.artifact_hash === null ? `解除课程「${args.lesson_id}」的工件绑定？` : `为课程「${args.lesson_id}」绑定工件？`, JSON.stringify({outlineId: args.outline_id, lessonId: args.lesson_id, artifactHash: args.artifact_hash}, null, 2), exec.signal)
                if (!answer.confirmed) return {outcome: 'cancelled' as const, ...(answer.custom === undefined ? {} : {detail: answer.custom})}
        ...
        async execute(args: {outline_id: string; phase: OutlinePhase; current_lesson_id?: string}, exec) {
            ...
            try {
                ...
                if (requiresConfirmation) {
                    const answer = await confirmWrite(rootCtx, agent, args.phase === 'completed' ? `结束「${before.title}」？` : `结束本课并进入下一课「${args.current_lesson_id ?? ''}」？`, JSON.stringify({from: before.workflow, to: {phase: args.phase, currentLessonId: args.current_lesson_id ?? null}}, null, 2), exec.signal)
                    if (!answer.confirmed) return {outcome: 'cancelled' as const, ...(answer.custom === undefined ? {} : {detail: answer.custom})}
                }
    ...
    disposers.push(toolCtx.tools.register(defineTool({
        ...
        async execute(args: {plan_id: string | null; outline_id?: string; lesson_id?: string; rating: ReviewRating}, exec) {
            ...
            try {
                ...
                const answer = await confirmWrite(rootCtx, agent, creating ? `为课程「${args.lesson_id ?? ''}」创建复习计划？` : `结算复习计划「${args.plan_id}」的当前期次？`, JSON.stringify({rating: args.rating}, null, 2), exec.signal)
                if (!answer.confirmed) return {outcome: 'cancelled' as const, ...(answer.custom === undefined ? {} : {detail: answer.custom})}
                if (creating) {
