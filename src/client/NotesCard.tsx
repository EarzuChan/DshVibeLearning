/**
 * The floating notes card (session header utilities slot): a collapsible
 * header expanding to the global note folders and their notes, with full
 * user-surface CRUD. Data arrives through the store seat (the state's `notes`
 * snapshot); mutations go through the injected `card.notes` face, which
 * re-fetches the state on every write.
 * @module dvl/client/NotesCard
 */
import { useState } from 'react'
import clsx from 'clsx'
import { Button, IconChevronDownOutline14, Modal } from '@deepseek-ai/dsh-client-ui-primitives'
import type { NotesCardProps } from './contract.ts'
import type { NoteAccess, NoteDto } from './types.ts'
import css from './NotesCard.module.css'

/** Access tiers in picker order (private last so the default is the least visible). */
const ACCESS_TIERS: readonly { value: NoteAccess; key: 'notes.access.readwrite' | 'notes.access.readable' | 'notes.access.private' }[] = [
  { value: 'readwrite', key: 'notes.access.readwrite' },
  { value: 'readable', key: 'notes.access.readable' },
  { value: 'private', key: 'notes.access.private' },
]

/** Editor draft: null noteId = new note. */
...
/**
 * Render the collapsible notes card.
...
export function NotesCard({ useStore, card, t }: NotesCardProps) {
  ...
  const notesByFolder = (folderId: string): readonly NoteDto[] =>
  ...
  const openEdit = (note: NoteDto): void => {
    setError(false)
    setEditor({
      noteId: note.id,
  ...
  const save = async (): Promise<void> => {
    ...
    try {
      ...
      if (editor.noteId === null) {
        ...
        await card.notes.updateNote(editor.noteId, {
          folderId: editor.folderId,
          title: editor.title,
          markdown: editor.markdown,
          tags,
  ...
  return (
    <div className={css.card}>
            ...
            : (
              <>
                <div className={css.toolbar}>
                  ...
                  <Button size="sm" variant="primary" disabled={folders.length === 0} onClick={openNew}>
                    {t('card.notes.addNote')}
                  </Button>
                </div>
                {folders.length === 0
                  ? <div className={css.empty}>{t('card.notes.empty')}</div>
                  : folders.map(folder => (
                    <div key={folder.id} className={css.folder}>
                      <div className={css.folderHeader}>
                        <span className={css.folderName}>{folder.name}</span>
                        <button type="button" className={css.miniButton} onClick={() => { renameFolder(folder.id, folder.name) }}>
                          {t('card.notes.rename')}
                        </button>
                        <button type="button" className={css.miniButton} onClick={() => { deleteFolder(folder.id) }}>
                          {t('card.notes.delete')}
                        </button>
                      </div>
                      {notesByFolder(folder.id).map(note => (
                        <div key={note.id} className={css.note}>
                          ...
                          <button type="button" className={css.miniButton} onClick={() => { openEdit(note) }}>
                            {t('notes.editor.title.edit')}
                          </button>
                          <button type="button" className={css.miniButton} onClick={() => { deleteNote(note.id) }}>
                            {t('card.notes.delete')}
                          </button>
                        </div>
                      ))}
      ...
      <Modal
        ...
        title={editor?.noteId === null ? t('notes.editor.title.new') : t('notes.editor.title.edit')}
        footer={(
          <>
            <Button variant="outline" disabled={busy} onClick={() => { setEditor(null) }}>{t('notes.cancel')}</Button>
            <Button variant="primary" disabled={busy} onClick={() => { void save() }}>{t('notes.save')}</Button>
          </>
        )}
      >
        {editor !== null && (
          <div className={css.editor}>
            ...
            <label className={css.field}>
              ...
              <input
                className={css.input}
                value={editor.title}
                placeholder={t('notes.note.title.placeholder')}
            ...
            <label className={css.field}>
              ...
              <input
                className={css.input}
                value={editor.tags}
                placeholder={t('notes.note.tags.placeholder')}
            ...
            <label className={css.field}>
              ...
              <select
                className={css.input}
                value={editor.access}
                onChange={event => { setEditor({ ...editor, access: event.target.value as NoteAccess }) }}
              >
