...
import { isValidArtifactHash, isValidRunId } from '../learning/files.ts'
...
/** Inject the base theme + submission bridge (no per-run/target state). */
...
export function installLearningRoutes(ctx: Context): void {
  ...
  const handle = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    // ── opaque submission ─────────────────────────────────────────
    // Run-scoped: the run identity is derived from the canonical URL, so
    // the request body is the raw JSON payload itself (no mechanism fields).
    ...
    if (req.method === 'POST' && submitRunMatch !== null) {
      ...
      const kind = categoryFromSegment(submitRunMatch[2])
    ...
    // Read-only preview has no run segment → its ./submit is rejected.
    ...
    // ── running-present descriptor resolver ────────────────────────
    ...
    // Active-run HTML: /learning/<ws>/<category>/<hash>/runs/<runId>/index.html
    ...
    if (req.method === 'GET' && runMatch !== null) {
      ...
      const kind = categoryFromSegment(runMatch[2])
    ...
    // Read-only preview HTML: /learning/<workspaceId>/<category>/<hash>/index.html
